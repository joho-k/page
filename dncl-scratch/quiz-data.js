window.quizData = {
    ...
    q057: {
        title: "60を素数のかけ算に分ける（素因数分解）",
        addedAt: "2026-09-11",
        difficulty: 5,
        question: "60 を素数のかけ算（60 ＝ 2 × 2 × 3 × 5）に分ける「素因数分解」をしよう。割る数 d を 2 から始めて、n が d で割り切れたら d を表示し、n を d で割った答えに置きかえます。割り切れなければ d を 1 ふやします。割り切れたときは d をふやさず、同じ d でもう一度割れるか調べるのがポイントです。2, 2, 3, 5 と最後の 5 まで表示されるように、くり返しの条件にも気をつけて3か所の穴をうめよう",
        ast: [
            {
                type: "assign",
                name: "n",
            ...
            {
                type: "while",
                condition: "d __BLANK_blank_a__ n",
                body: [
                    {
                        type: "assign",
                        name: "amari",
                        value: "n __BLANK_blank_b__ d"
                    },
                    {
                        type: "ifelse",
                        condition: "amari == 0",
        ...
        answers: [
            ...
            },
            {
                values: ["<", "%", "/"],
                correct: false,
                hint: "割り算の2か所は正解です。でも < だと、d と n が同じ数になったところでくり返しが終わってしまいます。n が 5 まで小さくなり、d が 5 まで進んだとき 5 < 5 は成り立たないので、最後の 5 が表示されず 2, 2, 3 で止まります。d と n が同じときも割ってみたいので <= を使います"
            },
            {
                values: ["<=", "%", "%"],
                correct: false,
                hint: "n = n % d にすると、n に「割ったあまり」が入ってしまいます。60 は 2 で割り切れるので n は 0 になり、d <= n（2 <= 0）が成り立たずにすぐ終わります。表示は 2 だけです。n を小さくしていくときは、割った答えを求める / を使います"
            },
            {
                values: ["<", "%", "%"],
                correct: false,
                hint: "2か所ちがいます。n = n % d だと n があまりの 0 になり、2 を1回表示しただけで終わります。さらに < だと、最後に d と n が同じ数になったときの 5 も表示できません。n を小さくするのは /、くり返しの条件は <= です"
            },
            {
                values: ["<=", "/", "/"],
                correct: false,
                hint: "amari = n / d にすると、amari には割った答え（60 ÷ 2 ＝ 30 など）が入ります。60 を割った答えが 0 になることはないので、amari == 0 が一度も成り立たず、d が 61 になるまで数えて何も表示されずに終わります。割り切れたかどうかは、あまりを求める % で調べます"
            },
            {
                values: ["<", "/", "/"],
                correct: false,
                hint: "amari = n / d では割った答えが入るので 0 にならず、何も表示されません。あまりを求めるのは % です。さらにくり返しの条件も < だと、d と n が同じになったときの最後の 5 が出せません。<= にしましょう"
            },
            {
                values: ["<=", "/", "%"],
                correct: false,
                hint: "/ と % が逆になっています。amari = n / d だと割った答えが入るので 0 にならず、何も表示されずに終わります。割り切れたか調べる（あまりを出す）のが %、n を割った答えに置きかえて小さくするのが / です"
            },
            {
                values: ["<", "/", "%"],
                correct: false,
                hint: "3か所ともちがいます。/ と % が逆なので amari が 0 にならず、何も表示されません。さらに < だと最後の 5 も出せません。くり返しは d <= n のあいだ、あまりは %、n を小さくするのは / です"
            }
        ],
        defaultHint: "素因数分解は「小さい数から順に、割れるだけ割る」をくり返します。割り切れたかどうかは、あまり（%）が 0 かどうかで調べます。割り切れたら d を表示して、n を割った答え（/）に置きかえます。このとき d はふやさず、同じ d でもう一度割れるか調べるので 2 が2回出ます。最後は d と n がどちらも 5 になったときも割りたいので、くり返しの条件は d <= n です。表示は 2, 2, 3, 5 の順です"
    },
}
