window.quizData = {
    ...
    q056: {
        ...
        addedAt: "2026-09-11",
        difficulty: 4,
        question: "「当たりが出る確率40%」のおみくじを作ろう。関数 omikuji は、乱数を使って 1 から 10 までのどれかの数を r に入れ、r が当たりの範囲におさまっていれば「当たり」、そうでなければ「はずれ」を返します。10個の数のうち4個が当たりになれば40%です。omikuji には当たりの個数を wariai としてわたします。2か所の穴をうめよう",
        ast: [
            {
                type: "func",
                name: "omikuji",
                ...
                body: [
                    ...
                    {
                        type: "if",
                        condition: "r __BLANK_blank_a__ wariai",
                        body: [
                            {
                                type: "return",
                                value: "\"当たり\""
                ...
                body: [
                    {
                        type: "assign",
                        name: "kekka",
                        value: "omikuji(__BLANK_blank_b__)"
                    },
                    {
                        type: "print",
                        value: "i + \"回目は\" + kekka"
        ...
        answers: [
            ...
            },
            {
                values: ["<", "4"],
                correct: false,
                hint: "< だと当たりになるのは 1・2・3 の3個だけで、30%になってしまいます。4 も当たりにふくめたいので、「以下」の <= を使います",
            },
            {
                values: [">=", "4"],
                correct: false,
                hint: "向きが逆です。>= だと 4・5・6・7・8・9・10 の7個が当たりになり、70%になってしまいます。当たりにしたいのは小さいほうの4個なので <= です",
            },
            {
                values: [">", "4"],
                correct: false,
                hint: "向きが逆です。> だと 5 から 10 までの6個が当たりになり、60%になってしまいます。1 から 4 までを当たりにする <= を使います",
            },
            {
                values: ["<=", "40"],
                correct: false,
                hint: "40% の「40」をそのまま入れたくなりますが、r は 1 から 10 までの数です。r <= 40 はいつでも成り立つので、10回中10回とも当たり＝100%になってしまいます。10個のうち何個を当たりにするかを考えて、4 を入れましょう",
            },
            {
                values: ["<=", "0.4"],
                correct: false,
                hint: "0.4 は「40%」を小数にしたものですが、r は 1 から 10 までの整数です。r <= 0.4 はいちども成り立たず、ぜんぶ「はずれ」になってしまいます。1 から 10 のうち4個を当たりにするので 4 です",
            },
            {
                values: ["<=", "10"],
                correct: false,
                hint: "r は必ず 10 以下なので、r <= 10 はいつでも成り立ちます。10回中10回とも当たり＝100%です。10個のうち4個だけを当たりにしたいので 4 を入れます",
            },
            {
                values: ["<", "10"],
                correct: false,
                hint: "2か所ちがいます。r < 10 だと 1 から 9 までの9個が当たりで90%です。当たりの個数は 4、そして 4 ちょうども当たりにふくめるので <= を使います",
            },
            {
                values: [">", "40"],
                correct: false,
                hint: "2か所ちがいます。r > 40 になる r はないので、ぜんぶ「はずれ」です。1 から 4 までを当たりにする <= と 4 の組み合わせにしましょう",
            }
        ],
        defaultHint: "切り捨て(乱数() * 10) + 1 は、1 から 10 までのどれかの数を作ります。10個のうち4個が当たりなら 4 わる 10 で40%。だから 1・2・3・4 を当たりにすればよく、4 もふくめるので r <= 4、つまり wariai には 4 をわたします。乱数を使っているので実行するたびに結果は変わりますが、何回もためすと4割くらいが「当たり」になります"
    },
}
