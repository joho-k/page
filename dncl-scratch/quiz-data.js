window.quizData = {
    ...
    q058: {
        title: "100日後は何曜日？（あまりでひとまわり）",
        addedAt: "2026-09-02",
        difficulty: 3,
        question: "曜日を配列 youbi に並べました（youbi[0] が日曜日、youbi[1] が月曜日…）。今日は水曜日なので youbi[3] です。曜日は7日でひとまわりすることを使って、今日から100日後が何曜日になるかを表示しよう（空欄は2つ）。",
        ast: [
            {
                type: "assign",
                name: "youbi",
            ...
            },
            {
                type: "assign",
                name: "saki",
                value: "(kyou + hi) __BLANK_blank_a__ __BLANK_blank_b__"
            },
            {
                type: "print",
                value: "\"今日から\" + hi + \"日後は\" + youbi[saki] + \"曜日です\""
            }
        ],
        choices: [
            { label: "%", value: "%" },
        ...
        answers: [
            ...
            },
            {
                values: ["%", "6"],
                correct: false,
                hint: "曜日は日・月・火・水・木・金・土の7つでひとまわりします。6 でわると 103 % 6 = 1 になり、youbi[1] の月曜日になってしまいます。わる数は曜日の個数と同じ 7 です"
            },
            {
                values: ["%", "24"],
                correct: false,
                hint: "24 は1日の時間の数です。いまひとまわりさせたいのは曜日なので、103 % 24 = 7 となり、配列 youbi の範囲（0〜6）からはみ出してしまいます。わる数は 7 です"
            },
            {
                values: ["/", "7"],
                correct: false,
                hint: "/ は商を出す記号なので 103 / 7 = 14.71… という小数になり、youbi の添字として使えません。ほしいのは「7でわったあまり」なので % を使います"
            },
            {
                values: ["/", "6"],
                correct: false,
                hint: "2か所ちがいます。ひとまわりの計算に使うのは、わり算の商（/）ではなく、あまり（%）です。そして曜日は7つなので、わる数も 7 になります"
            },
            {
                values: ["*", "7"],
                correct: false,
                hint: "* でかけると 103 * 7 = 721 になり、どんどん大きくなってしまいます。ひとまわりさせたいときは、7 でわったあまり（%）を使うと 0〜6 におさまります"
            }
        ],
        defaultHint: "水曜日は youbi[3] なので、3 + 100 = 103 が「日曜日から数えた番号」になります。曜日は7日でひとまわりするので、103 を 7 でわったあまりを出せば 0〜6 の添字にもどせます。あまりを出す記号は % です"
    },
}
