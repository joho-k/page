window.quizData = {
    ...
    q057: {
        title: "パスワードは何通り？（4けたの数字）",
        addedAt: "2026-09-01",
        difficulty: 2,
        question: "0から9までの数字が使える「4けたのパスワード」は、全部で何通り作れるでしょうか。1けたなら10通り、2けたならそのそれぞれにまた10通りずつ枝分かれ…とけたが増えるごとに通り数はふくらんでいきます。通り数を入れておく箱 kumi に、1けたぶんずつためていくプログラムです。kumi の最初の値と、繰り返しの中の計算をうめよう（2か所の穴をうめよう）",
        ast: [
            { type: "assign", name: "shurui", value: "10" },
            { type: "assign", name: "kumi", value: "__BLANK_blank_a__" },
            {
                type: "for",
                varName: "i",
                start: "1",
                ...
                step: "1",
                body: [
                    { type: "assign", name: "kumi", value: "kumi __BLANK_blank_b__ shurui" },
                ],
            },
            { type: "print", value: "\"4けたの数字のパスワードは\" + kumi + \"通りです\"" },
        ],
        choices: [
            { label: "0", value: "0" },
            { label: "1", value: "1" },
        ...
        answers: [
            { values: ["1", "*"], correct: true },
            {
                values: ["0", "*"],
                correct: false,
                hint: "0 に何をかけても 0 のままなので、4回くり返しても kumi は 0 通りのままです。たし算の合計は 0 から始めますが、かけ算をためていくときは 1 から始めます",
            },
            {
                values: ["10", "*"],
                correct: false,
                hint: "最初から 10 が入っていると、そこにさらに10を4回かけて 100000 通り（5けたぶん）になってしまいます。10をかけるのは4回だけでよいので、最初は 1 です",
            },
            {
                values: ["1", "+"],
                correct: false,
                hint: "+ だと 1 + 10 + 10 + 10 + 10 ＝ 41 通りにしかなりません。けたが1つ増えるたびに、それまでの1通り1通りが10通りずつに枝分かれするので、たし算ではなくかけ算です",
            },
            {
                values: ["1", "-"],
                correct: false,
                hint: "- だと 1 から10を4回ひいて -39 通りになってしまいます。通り数はけたが増えるほど多くなるので、ひき算ではなくかけ算です",
            },
            {
                values: ["1", "/"],
                correct: false,
                hint: "/ だと 1 を10で4回わって 0.0001 になってしまいます。けたが増えるほど通り数は多くなるので、わり算ではなくかけ算です",
            },
            {
                values: ["0", "+"],
                correct: false,
                hint: "2か所とも違います。+ では 0 + 10 + 10 + 10 + 10 ＝ 40 通りにしかなりません。かけ算を 1 からためていくのが正しい形です",
            },
            {
                values: ["10", "+"],
                correct: false,
                hint: "10 + 10 + 10 + 10 + 10 ＝ 50 通りです。4けたのパスワードは50通りよりずっと多く作れます。1 から始めて10をかけていきましょう",
            },
            {
                values: ["0", "-"],
                correct: false,
                hint: "0 から10を4回ひいて -40 通りになります。通り数がマイナスになることはありません。1 から始めて10をかけていきましょう",
            },
            {
                values: ["10", "-"],
                correct: false,
                hint: "10 から10を4回ひいて -30 通りになります。けたが増えるほど通り数は多くなるので、かけ算を使います",
            },
            {
                values: ["0", "/"],
                correct: false,
                hint: "0 を10でわっても 0 のままです。かけ算をためていくときの最初の値は 1、計算は * です",
            },
            {
                values: ["10", "/"],
                correct: false,
                hint: "10 を10で4回わると 0.001 になってしまいます。通り数を増やしたいので * を使い、最初は 1 から始めます",
            },
        ],
        defaultHint: "かけ算をためていくときは、最初の箱に 1 を入れます（0 だと何をかけても 0 のままです）。1けたごとに10通りずつ枝分かれするので、10 を4回かけて 10 * 10 * 10 * 10 ＝ 10000通りになります",
    },
}
