window.quizData = {
    ...
    q047: {
        title: "同じ数が続いたら短くする（ランレングス圧縮）",
        addedAt: "2026-08-21",
        difficulty: 5,
        question: "白黒画像の1行ぶんのデータ data（＝[1,1,1,1,1,1,0,0,0,1,1,0]、1が黒・0が白）を、ランレングス圧縮でまとめよう。ランレングス圧縮は「同じ数が何個続いたか」を数えて「1が6個」「0が3個」…と短く表す方法です。前の数を ima、続いた個数を kosuu にとっておき、前と同じ数ならふやし、ちがう数になったらそこまでの組を表示して数え直します。最後に、圧縮後がいくつの数で表せるかも計算します。1組は「1が6個」のように「どの数か」「何個か」の2つの数で表すので、圧縮後の数の個数は組の数 kumi から求められます。あてはまる記号を選ぼう（3か所の穴をうめよう）",
        ast: [
            {
                type: "assign",
                name: "data",
                value: "[1,1,1,1,1,1,0,0,0,1,1,0]"
            },
            {
                type: "assign",
                name: "ima",
            ...
            },
            {
                ...
                body: [
                    {
                        type: "ifelse",
                        condition: "data[i] __BLANK_blank_a__ ima",
                        ifBody: [
                            {
                                type: "assign",
                                name: "kosuu",
            ...
            },
            {
                type: "assign",
                name: "atokazu",
                value: "kumi __BLANK_blank_c__ 2"
            },
            {
                type: "print",
                value: "\"組の数は\" + kumi + \"組です\""
            ...
            {
                type: "print",
                value: "\"もとの12個の数が、圧縮後は\" + atokazu + \"個の数になりました\""
            }
        ],
        choices: [
            { label: "==", value: "==" },
        ...
        answers: [
            ...
            },
            {
                values: ["!=", "+", "*"],
                correct: false,
                hint: "!= は「前とちがうとき」なので、まとめる側とやり直す側が入れかわります。data[1] は前と同じ1なので、いきなり「1が1個」と表示して数え直しになり、6個続いた1が1個ずつばらばらに出てしまいます。続きをまとめたいので、前と同じかどうかを見る == を使います",
            },
            {
                values: [">", "+", "*"],
                correct: false,
                hint: "> は大小をくらべる記号です。0と1しかないデータでは 1 > 1 も 0 > 0 も成り立たないので、同じ数が続いていても毎回ちがう組としてやり直してしまいます。「前と同じ数か」を調べるのは == です",
            },
            {
                values: ["==", "-", "*"],
                correct: false,
                hint: "- だと同じ数が続くたびに kosuu が1ずつ減ります。1が6個続いたところで kosuu は -4 になり、「1が-4個」とマイナスの個数が表示されてしまいます。個数を数えるのは + です",
            },
            {
                values: ["==", "*", "*"],
                correct: false,
                hint: "kosuu * 1 は何回やっても 1 のままです。1が6個続いても「1が1個」と表示され、まったく数えられていません。1ずつふやす + にしましょう",
            },
            {
                values: ["==", "+", "+"],
                correct: false,
                hint: "1つの組は「どの数か」と「何個か」の2つの数で表します。組は4つなので圧縮後は8個の数ですが、+ だと 4 + 2 ＝ 6 個となり合いません。1組につき2つ分なので、かけ算の * です",
            },
            {
                values: ["==", "+", "/"],
                correct: false,
                hint: "/ だと 4 / 2 ＝ 2 個となり、4つの組を2つの数で表せることになってしまいます。1組は数と個数の2つ組なので、組の数に2をかけて 4 * 2 ＝ 8 個と求めます",
            },
            {
                values: ["!=", "+", "/"],
                correct: false,
                hint: "2か所ちがいます。まとめられるのは「前と同じ数」のときなので ==、圧縮後の数の個数は1組あたり2つ分で 組の数 * 2 です",
            }
        ],
        defaultHint: "コツは「直前の数 ima」と「続いた個数 kosuu」をとっておくことです。data[i] == ima なら kosuu + 1 で個数をふやし、ちがえばそこまでの組を表示して数え直します。このデータは 1が6個・0が3個・1が2個・0が1個 の4組になります。1組は「どの数か」「何個か」の2つの数で表すので、圧縮後は 4 * 2 ＝ 8個の数となり、もとの12個より短くなります",
    },
}
