window.quizData = {
    ...
    q063: {
        ...
        addedAt: "2026-09-09",
        difficulty: 3,
        question: "5つの店の値段を配列 nedan（＝[320,280,350,260,300]）に入れました。この中でいちばん安い値段を見つけて表示しよう。saiyasu に「今のところ一番安い値段」を入れておき、店を1つずつ見ながら、それより安ければ saiyasu を入れかえます。ポイントは saiyasu の最初の値です。合計を数えるときのように 0 から始めてよいか、よく考えよう（2か所の穴をうめよう）",
        ast: [
            {
                type: "assign",
                name: "nedan",
        ...
        answers: [
            ...
            },
            {
                values: ["nedan[0]", "=="],
                correct: false,
                hint: "== は「等しいか」を調べる記号なので、安いか高いかをくらべられません。nedan[i] == saiyasu が成り立つのは自分自身とくらべた最初の1回だけで、saiyasu は 320 のまま終わります"
            },
            {
                values: ["0", "<"],
                correct: false,
                hint: "合計を数えるときのくせで 0 から始めると失敗します。0 より安い店は1つもないので、if の中が一度も実行されず、答えは 0 円のままです。最初は「1つ目の店の値段」を入れておきましょう"
            },
            {
                values: ["0", ">"],
                correct: false,
        ...
        defaultHint: "最小値さがしは「まず1つ目を仮のチャンピオンにして、あとから来た相手が強ければ（安ければ）入れかえる」と考えます。だから saiyasu = nedan[0] から始め、もし nedan[i] < saiyasu なら入れかえます。合計のように 0 から始めると、0 より安い店は無いので 0 のまま終わってしまいます。答えは 260 円です",
    },
}
